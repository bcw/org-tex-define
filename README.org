...
and the [[file:examples/]] directory currently contains one extended
example:

- An explanation of how [[file:examples/rotation.org][rotation matrices]] can be derived by purely
  algebraic methods.
